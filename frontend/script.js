...
function initGoogleSignIn() {
  ...
  const missingNote = (text) => {
    if (googleNote) {
      googleNote.textContent = text;
      googleNote.style.display = 'block';
    }
  };
  ...
  if (!clientId) {
    ...
    missingNote('Google login is not configured. Please set a valid Google Client ID.');
    return;
  }
  ...
  if (isFileProtocol) {
    ...
    googleWrap.style.display = 'none';
    missingNote('Google login requires serving the app over HTTP(S). Run a local server and open the app via localhost.');
    return;
  }

  if (!window.google?.accounts?.id) {
    // Google script not loaded yet, retry in 1 second
    setTimeout(initGoogleSignIn, 1000);
    setupBtn.style.display = 'inline-flex';
    missingNote('Loading Google sign-in...');
    return;
  }
...
function initAuth() {
  ...
  $('#googleSetupBtn')?.addEventListener('click', () => {
    initGoogleSignIn();
    showToast('Attempting to load Google sign-in...');
  });
