...
async function generateResults() {
  ...
  // Send the raw recipient value — the backend maps it to DB values
  const recipient = state.inputs.recipient || null;
  ...
  try {
    ...
    if (ok && data.success && data.data.results.length > 0) {
      ...
      const gifts = data.data.results.map(g => ({
        ...
        reason: g.aiReason || g.reason || `${g.category || 'Gift'} · Rs. ${g.price?.toLocaleString() || ''}`,
